...
| 模型              | mAP@0.5   | mAP@0.5:0.95 | 参数量 (M) | FLOPs (G) | 模型大小 (MB) |
| ----------------- | --------- | ------------ | ---------- | --------- | ------------- |
| Faster R-CNN      | 55.7%     | 20.3%        | 40.1       | 34.7      | 339.7         |
| SSD               | 51.2%     | 29.4%        | 29.51      | 30.1      | 169.72        |
| YOLOv5n           | 84.3%     | 79.2%        | 3.23       | 7.6       | 7.81          |
| YOLOv6n           | 88.4%     | 82.7%        | 4.82       | 12.2      | 8.32          |
| YOLOv7-tiny       | 90.3%     | 83.1%        | 6.21       | 11.7      | 10.81         |
| YOLOv9t           | 93.5%     | 88.6%        | 3.14       | 12.1      | 5.1           |
| YOLOv10n          | 92.8%     | 87.9%        | 2.81       | 8.1       | 5.71          |
| **YOLO-PLNet** 🏆 | **98.1%** | **94.7%**    | **2.13**   | **5.4**   | **4.51**      |

> 💡 **性能亮点**：YOLO-PLNet 在所有对比模型中取得最佳性能，相比 YOLOv10n 提升 5.3% mAP@0.5，同时参数量减少 24%，模型大小减少 21%

### 边缘设备部署性能

#### Jetson Orin NX 平台量化对比

| 量化精度 | 延迟 (ms/img) | FPS (frame/s) | GPU 内存占用 (%) | 功耗 (W) | mAP@0.5 |
| -------- | ------------- | ------------- | ---------------- | -------- | ------- |
| FP32     | 32.5          | 15.4          | 62.3             | 5.2      | 98.1%   |
| FP16     | 19.1          | 28.2          | 52.1             | 4.1      | 98.0%   |
| INT8     | 11.8          | 41.3          | 48.6             | 3.4      | 97.5%   |

> 📱 **部署优势**：INT8 量化后推理速度提升 168%，功耗降低 35%，精度仅下降 0.6%

#### 不同输入分辨率性能测试

| 输入分辨率 | 延迟 (ms/img) | FPS (frame/s) | 功耗 (W) |
| ---------- | ------------- | ------------- | -------- |
| 320×320    | 16.7          | 52.4          | 3        |
| 640×640    | 19.1          | 28.2          | 4.1      |
| 1280×1280  | 32.6          | 13.4          | 6        |

### 病害检测详细结果

根据论文中的 Precision-Recall 曲线和混淆矩阵，模型在 6 个类别上的综合 mAP@0.5 达到 **96.8%**，各类别检测性能优异：

| 病害类别                        | 检测表现               |
| ------------------------------- | ---------------------- |
| 早期叶斑病 (Early Leaf Spot)    | 高精度识别             |
| 早期锈病 (Early Rust)           | 完美分类 (100% 准确率) |
| 晚期叶斑病 (Late Leaf Spot)     | 稳定检测               |
| 营养缺乏 (Nutrition Deficiency) | 良好区分能力           |
| 锈病 (Rust)                     | 高召回率               |
| 健康叶片 (Healthy)              | 准确识别               |

> 🎯 **实际应用**：模型已在 Jetson Orin NX 平台上成功部署，实现了 640×640 分辨率下 28.2 FPS 的实时检测，适合田间实际应用
...
@article{sun2025yolo,
  title={YOLO-PLNet: a lightweight real-time detection model for peanut leaf diseases based on edge deployment},
  author={Sun, Jinti and Feng, Zhihui and Han, Jiaqi and Xu, Fulei and Zhang, Hui and Guo, Yufeng},
